...
class DataSource(str, Enum):
    ...
    def loader(self) -> DataLoader:
        """获取对应的数据加载器"""
        loaders = {
            DataSource.BHDS: BHDSLoader(),
            DataSource.QUANTCLASS: QuantClassLoader()
        }
        return loaders[self]
...
class CompareField(str, Enum):
    ...
    def get_fields_by_trade_type(cls, trade_type: TradeType) -> List['CompareField']:
        ...
        base_fields = [
            ...
            cls.VOLUME, cls.QUOTE_VOLUME, cls.TRADE_NUM,
            cls.TAKER_BUY_BASE, cls.TAKER_BUY_QUOTE
        ]
...
class CompareResult:
    ...
    def __str__(self) -> str:
        return f"字段: {self.field}, 最大差异: {self.max_diff}, 差异数量: {self.diff_num}"
...
class DataComparer:
    ...
    def _align_time_range(self):
        """对齐两个数据源的时间范围"""
        begin_ts = max(self.df_bhds['candle_begin_time'].min(),
                       self.df_qtc['candle_begin_time'].min())
        end_ts = min(self.df_bhds['candle_begin_time'].max(),
                     self.df_qtc['candle_begin_time'].max())
    ...
    def compare_field(self, field: CompareField) -> CompareResult:
        ...
        df = self.df_bhds.merge(
            self.df_qtc[['candle_begin_time', field.value]], 
        ...
        diff_details = df[df['diff_abs'] > field.diff_threshold][
            ['candle_begin_time', field.value, f'{field.value}_qtc', 'diff']
        ]
    ...
    def get_time_range(self) -> Tuple[datetime, datetime]:
        ...
        return (
            ...
            self.df_bhds['candle_begin_time'].max()
        )
