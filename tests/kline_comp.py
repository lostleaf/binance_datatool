...
def test_detection_only(data_dir: Path, symbols: list[str], interval: str = "1m"):
    ...
    print("🔍 Testing DailyKlineDetector.detect...")
...
async def test_data_executor(data_dir: Path, symbols: list[str], interval: str = "1m"):
    ...
    async with create_aiohttp_session(HTTP_TIMEOUT_SEC) as session:
        ...
        for _, _, save_path in tasks[:10]:
            if save_path.exists():
                print(f"  ✅ {save_path.name} created successfully")
                try:
                    df = pl.read_parquet(save_path)
                    print(f"     Shape: {df.shape}, Columns: {len(df.columns)}")
                    if len(df) > 0:
                        print(
                            f"     Date range: {df['candle_begin_time'].min()} ~ {df['candle_begin_time'].max()}"
                        )
                except Exception as e:
                    print(f"     Error reading file: {e}")
            else:
                print(f"  ❌ {save_path.name} not found")
...
async def test_full_workflow(data_dir: Path, symbols: list[str], interval: str = "1m"):
    ...
    print("🔄 Testing full workflow: detection + execution...")
    
    # Step 1: Detection
    print("Step 1: Detecting missing data...")
    tasks = test_detection_only(data_dir, symbols, interval)
    
    if not tasks:
        print("✅ No missing data detected, workflow complete")
        return {"detected": 0, "executed": 0, "successful": 0}
    
    # Step 2: Execution
    print("\nStep 2: Executing completion tasks...")
...
async def test_api_kline_detector():
    ...
    print("=" * 80)
    print("Testing API Kline Detector and Executor Modules")
    print("=" * 80)
    ...
    with tempfile.TemporaryDirectory(prefix="api_kline_detector_test_") as temp_dir:
        ...
        try:
            ...
            print("\n" + "-" * 60)
            print("TEST 3: Full Workflow (Detection + Execution)")
            print("-" * 60)
    ...
    print("\n" + "=" * 80)
    print("✅ API Kline Detector and Executor Test completed")
    print("=" * 80)
