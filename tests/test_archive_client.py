...
@pytest.mark.asyncio
async def test_archive_client_list_symbol_files_batch_reports_progress(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Batch listings should report progress as symbols finish."""
    client = ArchiveClient()
    ...
    async def fake_list_symbol_files(
        ...
        *,
        ...
        del trade_type, data_freq, data_type, interval, session
        await asyncio.sleep(0.01 if symbol == "BTCUSDT" else 0)
        return []

    monkeypatch.setattr(client, "_create_session", lambda: FakeSession())
    monkeypatch.setattr(client, "list_symbol_files", fake_list_symbol_files)
    monkeypatch.setattr("binance_datatool.bhds.archive.client.make_reporter", fake_make_reporter)
    ...
    assert list(result) == ["BTCUSDT", "ETHUSDT"]
    assert captured == {
        "progress_bar": True,
        "total": 2,
        "description": "List files",
    }
    assert events == [("ETHUSDT", True, 1), ("BTCUSDT", True, 1)]
