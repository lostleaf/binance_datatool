...
async def async_retry_getter(func, max_times=5):
    sleep_seconds = 1
    while True:
        try:
            return await func()
...
def next_run_time(time_interval):
    ...
    """
    ti = parse_interval_str(time_interval)
...
def parse_interval_str(time_interval):
    if time_interval[-1] in ('m', 'T'):  # minutes
        return timedelta(minutes=int(time_interval[:-1]))

    if time_interval[-1] in ('h', 'H'):  # hours
        return timedelta(hours=int(time_interval[:-1]))

    raise ValueError(f'{time_interval} format not supported')